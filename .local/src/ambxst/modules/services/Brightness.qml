...
Singleton {
    ...
    Process {
        ...
        stdout: SplitParser {
            ...
            onRead: data => {
                ...
                const connectorLine = lines.find(l => l.startsWith("DRM connector:") || l.startsWith("DRM_connector:"));
                ...
                let connector = "";
                if (connectorLine) {
                    const connRaw = connectorLine.split(":").slice(1).join(":").trim();
                    const connMatch = connRaw.match(/^card\d+-(.+)$/);
                    connector = connMatch ? connMatch[1] : connRaw;
                }
    ...
    component BrightnessMonitor: QtObject {
        ...
        readonly property var ddcEntry: {
            ...
            if (screenName) {
                const connectorMatch = root.ddcMonitors.find(entry => entry.connector && entry.connector === screenName && !usedBuses.includes(entry.busNum));
                if (connectorMatch)
            ...
            if (screenModel) {
                const modelMatch = root.ddcMonitors.find(entry => {
                    if (!entry.model || usedBuses.includes(entry.busNum))
                        return false;
                    const entryModel = entry.model.toLowerCase();
                    return entryModel === screenModel || entryModel.includes(screenModel) || screenModel.includes(entryModel);
                });
                if (modelMatch)
                    return modelMatch;
            }
